...
# Output files
path_out <- "../data/ad_tone_mentionbased_google_2020.csv"

# Read Google 2020 variables file for aws_face
aws <- fread(path_google_2020_vars, encoding = "UTF-8") %>%
...
el <- select(el, -advertiser_name_detected_entities)
...
df <- df %>% 
  filter(bucket == "1") %>%
  select(ad_id, advertiser_id, fecid_formerge, all_unique_entities)
...
df$ad_tone_entities <- 
  lapply(df$ad_tone_entities, function(x){x[!x %in% pol$fec_ids]}) %>%
